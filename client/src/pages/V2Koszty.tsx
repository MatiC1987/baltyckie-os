...
function OperationalCostsTab({ data, currentMonth }: { data: CostsSummaryResponse; currentMonth: number }) {
  const categories = Object.entries(data.operationalCosts);
        ...
        <table className="w-full text-xs border-collapse">
          ...
          <tbody>
            {categories.map(([catId, months]) => {
              ...
              return (
                <tr key={catId} className="border-b" data-testid={`op-cost-row-${catId}`}>
    ...
    <div className="space-y-4" data-testid="v2-koszty-page">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="operacyjne">
          {data && <OperationalCostsTab data={data} currentMonth={year === currentYear ? currentMonth : -1} />}
        </TabsContent>
