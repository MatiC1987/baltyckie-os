...
export default function ZadaniaLogin() {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [loading, setLoading] = useState(false);
  const { login } = useZadaniaAuth();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 p-4">
      <Card className="w-full max-w-sm p-8 space-y-6" data-testid="card-zadania-login">
        <div className="flex flex-col items-center gap-4">
          <img src={logoImg} alt="Logo" className="h-16 w-auto rounded-lg" />
          <div className="text-center">
            <h1 className="text-xl font-bold text-foreground" data-testid="text-zadania-title">Bałtyckie - Panel Zadań</h1>
            <p className="text-sm text-muted-foreground mt-1">Panel pracownika</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="zadania-email">Email</Label>
            <Input
              id="zadania-email"
              type="email"
              value={email}
              ...
              data-testid="input-zadania-email"
              autoComplete="email"
              className="text-[16px]"
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="zadania-password">Hasło</Label>
            <Input
              id="zadania-password"
              type="password"
              value={password}
              ...
              placeholder="••••••••"
              data-testid="input-zadania-password"
              autoComplete="current-password"
              className="text-[16px]"
            />
          </div>
          <Button type="submit" className="w-full" disabled={loading} data-testid="button-zadania-login">
