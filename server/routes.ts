  ...
  app.post('/api/subleases/generate-contract', isAuthenticated, async (req, res) => {
    try {
      ...
      const replacements: Record<string, string> = {
        ...
        "NUMER_LOKALU": aptNames.length > 1 ? aptNames.slice(0, -1).join(", ") + " oraz " + aptNames[aptNames.length - 1] : aptNames[0] || "",
        "ADRES_LOKALU": aptNames.length > 1 ? aptNames.slice(0, -1).join(", ") + " oraz " + aptNames[aptNames.length - 1] : aptNames[0] || "",
        "LICZBA_LOKALI": String(aptNames.length),
