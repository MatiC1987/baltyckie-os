...
*   **Cennik (Pricing Module):** Calendar-based daily price management per apartment. Features: `daily_prices` table with per-day pricing, source tracking (manual/rule/auto/hotres), min/max stay rules, blocked dates. `pricing_rules` table for seasonal, weekend, day-of-week, last-minute, long_stay, holiday, occupancy rules with percentage/fixed modifiers, priority ordering, auto-apply. `price_change_history` audit trail. `minPrice`/`maxPrice` constraints on apartments. Price Calendar page (`/price-calendar`) with month/week view, inline editing, bulk edit, color-coded sources, location filter, price history dialog. Pricing Rules page (`/pricing-rules`) with CRUD, preview impact before applying, activate/deactivate toggle. Pricing Analytics page (`/pricing-analytics`) with KPI cards (ADR, RevPAR, occupancy, pickup, conversion), per-apartment analytics table with r/r comparison, price history chart, alerts engine (low occupancy, price drops, booking pace), CSV export. AI Pricing page (`/ai-pricing`) with recommendation generation, accept/reject/bulk-accept workflow, auto-apply mode per apartment, what-if price simulator, `ai_recommendations` + `ai_pricing_config` tables. Competitor Monitoring page (`/competitors`) with competitor CRUD, manual rate entry, CSV import, comparison table with color-coded price positioning, trend charts (Recharts), price alerts, `competitor_properties` + `competitor_rates` tables. Holidays table (`holidays`) with 15 Polish holidays pre-seeded (including Polish Easter calculation). HotRes sync with cleaning fee support, scheduler for rule application. Sidebar section: CENNIK (yellow) with 5 items: price-calendar, pricing-rules, pricing-analytics, ai-pricing, competitors.
