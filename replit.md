...
-   **Saldo Firmowe** (`/saldo-firmowe`): 60-month rolling company balance forecast. API: `GET /api/balance-forecast` — computes from current account balances (all account_snapshots latest per account), revenue_forecasts (0-indexed months), actual revenue from reservations.paidAmount (PRZYJETA/ZAMELDOWANY), apt_cost_data, op_cost_data. Year fallback: if year Y has no data, uses Y−1 same month. UI: 3 tiles (current/max/min balance), Recharts AreaChart with gradient, monthly table grouped by year (P=Prognoza, R=Rzeczywiste, Poz=Pozostałe columns). View toggle: Oba/Wykres/Tabela.
