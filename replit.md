...
-   **Loans Management (Pożyczki)**: Clickable "Pożyczki" account in dashboard company balance card opens LoansDialog. Full CRUD for loans (title, debtor, amount, status, notes). Partial payment support with payment history. Auto-calculated total from active loans replaces manual account balance. Progress bars showing repayment percentage. Tables: `loans`, `loan_payments`.
