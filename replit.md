...
*   **Document & Workflow Automation:** AI-powered PDF contract import (GPT-4o vision OCR), Word contract generation, invoice generation, cost invoice management, and automated PDF generation for accounting notes and handover protocols. **Document Templates** use `templateType` field (UMOWA/ANEKS/NOTA). **ANEKS template** was seeded from user's real DOCX reference (scripts/seed-annex-template.ts) — processes with pizzip+mergeRunsAndReplace, replacing Elektropaks-specific values with [PLACEHOLDER] format. Startup seed runs in production via `server/seed-aneks-template.ts` (called from index.ts after schema/data migrations). New placeholder: [ADRES_ZAMIESZKANIA_WYNAJMUJACEGO] → companyFullAddress in generate-annex and generate-contract routes.
